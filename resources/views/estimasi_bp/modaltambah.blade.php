...
<div class="modal-dialog" style="max-width: 90%;">
        ...
        <div class="modal-body">
            ...
            <div class="tab-content">
                <div role="tabpanel" class="tab-pane fade in show active" id="pagemobil">
                    <div class="card-body">
                        <form action="tbmobil_update" method="post" class="formestimasi_bp">
                            @csrf
                            @if ($tbmobil->id)
                                @method('post')
                            @endif
                    ...
                    <div class="card-body">
                        {{-- <form action="tbmobil_update" method="post" class="formestimasi_bp">
                            ...
                            @if ($tbmobil->id)
                                @method('post')
                            @endif --}}
                        <table id="tbl-estimasi_bp" class="table table-bordered table-hover table-sm tbl-estimasi_bp">
                            <thead>
                                <tr>
                                    <th width="30">#</th>
    ...
    <script>
        ...
        function reload_table_estimasi_bp() {
            ...
            $(function() {
                var table = $('.tbl-estimasi_bp').DataTable({
                    ajax: "{{ url('estimasi_bpajax') }}?nopolisi=" + vnopolisi,
                    type: "GET",
                    ...
                    columns: [{
                        ...
                        },
                        {
                            ...
                            // data: null,
                            render: function(data, type, row, meta) {
                                return `<a href="#" onclick="detail_estimasi_bp(${row.id})">${row.noestimasi}</a>`;
                            }
                        },
                        {
                            data: 'tanggal',
        ...
        function reload_table_wo_bp() {
            ...
            $(function() {
                var table = $('.tbl-wo_bp').DataTable({
                    ajax: "{{ url('wo_bpajax') }}?nopolisi=" + vnopolisi,
                    type: "GET",
                    // beforeSend: function(jqXHR) {
                    //     jqXHR.overrideMimeType("application/json");
                    //     $('#p1').append('<h3> The beforeSend() function called. </h3>');
        ...
        $(document).ready(function() {
            $('.formestimasi_bp').submit(function(e) {
                const form = $(this)
                e.preventDefault();
                $.ajax({
                    type: "POST",
        ...
        $('.btntambahestimasi').click(function(e) {
            ...
            e.preventDefault();
            $.ajax({
                // url: `{{ route('estimasi_bp_create') }}`,
                url: `{{ route('estimasi_bp.create') }}`,
                dataType: "json",
                // type: "POST",
                data: {
                    id: id,
        ...
        function detail_estimasi_bp(id) {
            $.ajax({
                type: "get",
        ...
        function cetakestimasi(id) {
            $url = "{{ url('cetak_estimasi_bp') }}?id=" + id,
                // data: {
                //     id: id,
                //     _method: "POST",
                //     _token: '{{ csrf_token() }}',
