...
// $queryh = mysqli_query($connect, "select * from estimasi_bp where id='$id' and estimasi_bp.close=1 order by noestimasi");
// $de = mysqli_fetch_assoc($queryh);
// $noestimasi = $de['noestimasi'];
// $tanggal = date('d-M-Y', strtotime($de['tanggal'])); //$de['tgljual']; //

$tanggal = date('d-M-Y', strtotime($estimasi_bp->tanggal)); //$estimasi_bp->tgljual']; //
$kdpemilik = $estimasi_bp->kdpemilik;
$nmpemilik = $estimasi_bp->nmpemilik;
$total = $estimasi_bp->total;
$keluhan = $estimasi_bp->keluhan;
$ppn = $estimasi_bp->pr_ppn;
$rp_ppn = $estimasi_bp->total * ($ppn / 100);
$total_estimasi = $estimasi_bp->total_estimasi;
$pemilik = $estimasi_bp->kdpemilik . '-' . $estimasi_bp->nmpemilik;
...
$html .=
    '<p style="font-family: Tahoma, Verdana, Segoe, sans-serif;font-size:18px;text-align:center;margin-top:1px;margin-bottom:0px;"><u>ESTIMASI BODY REPAIR</u></p>
	<table border="0">
    <tr>
      <td colspan=4 style="font-size:18px;font-family: Tahoma, Verdana, Segoe, sans-serif;text-align:center;">&nbsp;&nbsp;NO.' .
    "$estimasi_bp->noestimasi" .
    '</td>
    </tr>
...
$totalppn = 0;
$totaljasa = 0;
foreach ($estimasi_bpd_jasa as $row) {
    $n++;
...
// if (count($estimasi_bpd_part) > 0 or count($estimasi_bpd_bahan) > 0) {
...
$totalppn = 0;
$totalpart = 0;
foreach ($estimasi_bpd_part as $row) {
    $n++;
    $fharga = number_format($row['harga'], 0, ',', '.');
    $fqty = number_format($row['qty'], 0, ',', '.');
    $fpr_discount = number_format($row['pr_discount'], 2, ',', '.');
...
foreach ($estimasi_bpd_bahan as $row) {
    $n++;
    $fharga = number_format($row['harga'], 0, ',', '.');
    $fqty = number_format($row['qty'], 0, ',', '.');
    $fpr_discount = number_format($row['pr_discount'], 2, ',', '.');
...
$html .=
    ...
    '</b></td></td></tr>';
$html .= '</tbody></table>';
$fown_risk = number_format($estimasi_bp->own_risk, 0, ',', '.');
$html .= '<br>Own Risk : Rp. ' . $fown_risk . ',-<br>';
$html .= '<font size=2><br>Jakarta, ' . date('d', strtotime($estimasi_bp->tanggal)) . ' ' . date('M', strtotime($estimasi_bp->tanggal)) . ' ' . date('Y', strtotime($estimasi_bp->tanggal)) . ',';
$html .=
