...
<div class="modal-dialog" style="max-width: 95%;">
    <div class="modal-content">
        ...
        <div class="modal-body">
            ...
            <form action={{ $action }} method="post" class="formtambahestimasi">
                @csrf
                @if ($estimasi_bp->id)
                    @method('put')
                @endif
                <input type="hidden" class="form-control form-control-sm" name="idestimasi" id="idestimasi"
                    value="<?= $estimasi_bp['id'] ?>">
                <div class="row mb-2 mt-0">
                    <div class="col-12 col-sm-6">
                        <?php
                        date_default_timezone_set('Asia/Jakarta');
                        $tglestimasi_bp = date('Y-m-d H:i:s');
                        ?>
                        <label for="nama" class="form-label mb-1">No. Estimasi / Tanggal
                            (M-D-Y)</label>
                        <input type='hidden' class='form-control form-control-sm mb-2'
                            value="{{ $estimasi_bp->noestimasi }}" readonly style="width: 5%" id='noestimasilama'
                            name='noestimasilama'>
                        <div class="input-group mb-1">
                            <input type='text' class='form-control form-control-sm mb-2'
                                value="{{ str_contains($vdata['title'], 'Tambah') ? 'AUTO GENERATE' : $estimasi_bp->noestimasi }}"
                                style="width: 15%" id='noestimasi'
                                {{ str_contains($vdata['title'], 'Edit') ? 'name=noestimasi' : '' }} readonly>
                            {{-- <input type='text' class='form-control form-control-sm mb-2'
                                value="{{ str_contains($vdata['title'], 'Tambah') ? 'AUTO GENERATE' : $estimasi_bp->noestimasi }}"
                                id="noestimasi" name="noestimasi" readonly style="width: 5%"> --}}
                            <input type="datetime-local" class='form-control form-control-sm mb-2' name='tanggal'
                                id='tanggal' value="<?= $tglestimasi_bp ?>" style="width: 30%" readonly>
                            <input type="datetime-local" class='form-control form-control-sm mb-2' name='est_selesai'
                                id='est_selesai' value="<?= $estimasi_bp->est_selesai ?>" style="width: 30%"
                        ...
                        <div class="input-group mb-2">
                            <select id='kdservice' name='kdservice' class="form-select form-select-sm"
                                ...
                                for ($c = 0; $c < $jml_kata; $c += 1) {
                                    if ($arrkdservice[$c] == $estimasi_bp->kdservice) {
                        ...
                        <div class="input-group mb-2">
                            <select id='aktifitas' name='aktifitas' class="form-select form-select-sm"
                                ...
                                for ($c = 0; $c < $jml_kata; $c += 1) {
                                    if ($arraktifitas[$c] == $estimasi_bp->aktifitas) {
                            ...
                            <select id='fasilitas' name='fasilitas' class="form-select form-select-sm"
                                ...
                                for ($c = 0; $c < $jml_kata; $c += 1) {
                                    if ($arrfasilitas[$c] == $estimasi_bp->fasilitas) {
                            ...
                            <select id='status_tunggu' name='status_tunggu' class="form-select form-select-sm"
                                ...
                                for ($c = 0; $c < $jml_kata; $c += 1) {
                                    if ($arrstatus_tunggu[$c] == $estimasi_bp->status_tunggu) {
                        ...
                        <div class="input-group mb-2">
                            <select id='int_reminder' name='int_reminder' class="form-select form-select-sm"
                                ...
                                for ($c = 0; $c < $jml_kata; $c += 1) {
                                    if ($arrint_reminder[$c] == $estimasi_bp->int_reminder) {
                        ...
                        <div class="input-group mb-2">
                            <input type="text" style="width:5%;" name="kdsa" id="kdsa"
                                value="<?= $estimasi_bp['kdsa'] ?>" class="form-control"
                                {{ str_contains($vdata['title'], 'Detail') ? 'readonly' : '' }}>
                            <input type="text" style="width:50%;" name="nmsa" id="nmsa"
                                value="<?= $estimasi_bp['nmsa'] ?>" class="form-control" readonly>
                        ...
                        <textarea class="form-control" name="keluhan" id="keluhan" rows="4"
                            {{ str_contains($vdata['title'], 'Detail') ? 'readonly' : '' }}>{{ $estimasi_bp->keluhan }}</textarea>
                        ...
                        <input type="number" class="form-control form-control-sm" style="text-align:right;"
                            name="own_risk" id="own_risk" value="<?= $estimasi_bp['own_risk'] ?>"
                            {{ str_contains($vdata['title'], 'Detail') ? 'readonly' : '' }}>
                    </div>
                    ...
                    <div class="col-12 col-sm-6">
                        ...
                        <textarea class="form-control" name="alamat_asuransi" id="alamat_asuransi_es" rows="2" readonly>{{ $estimasi_bp['alamat_asuransi'] }}</textarea>
                        <label for="nama" class="form-label mb-1">Surveyor</label>
                        <input type="text" class="form-control form-control-sm mb-2" name="surveyor"
                            id="surveyor" value="<?= $estimasi_bp['surveyor'] ?>"
                            {{ str_contains($vdata['title'], 'Detail') ? 'readonly' : '' }}>
                        ...
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="checkbox" id="klaim" name="klaim"
                                {{ (str_contains($vdata['title'], 'Tambah') ? 'checked' : $estimasi_bp->klaim == '1') ? 'checked' : '' }}
                                {{ str_contains($vdata['title'], 'Detail') ? 'disabled' : '' }}>
                        ...
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="checkbox" id="internal" name="internal"
                                {{ (str_contains($vdata['title'], 'Tambah') ? 'checked' : $estimasi_bp->internal == '1') ? 'checked' : '' }}
                                {{ str_contains($vdata['title'], 'Detail') ? 'disabled' : '' }}>
                        ...
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="checkbox" id="inventaris" name="inventaris"
                                {{ (str_contains($vdata['title'], 'Tambah') ? 'checked' : $estimasi_bp->inventaris == '1') ? 'checked' : '' }}
                                {{ str_contains($vdata['title'], 'Detail') ? 'disabled' : '' }}>
                        ...
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="checkbox" id="campaign" name="campaign"
                                {{ (str_contains($vdata['title'], 'Tambah') ? 'checked' : $estimasi_bp->campaign == '1') ? 'checked' : '' }}
                                {{ str_contains($vdata['title'], 'Detail') ? 'disabled' : '' }}>
                        ...
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="checkbox" id="booking" name="booking"
                                {{ (str_contains($vdata['title'], 'Tambah') ? 'checked' : $estimasi_bp->booking == '1') ? 'checked' : '' }}
                                {{ str_contains($vdata['title'], 'Detail') ? 'disabled' : '' }}>
                        ...
                        <div class="form-check form-check-inline">
                            <input class="form-check-input mb-2" type="checkbox" id="lain_lain" name="lain_lain"
                                {{ (str_contains($vdata['title'], 'Tambah') ? 'checked' : $estimasi_bp->lain_lain == '1') ? 'checked' : '' }}
                                {{ str_contains($vdata['title'], 'Detail') ? 'disabled' : '' }}>
                        ...
                        <input type="number" class="form-control form-control-sm mb-2" name="pr_ppn"
                            id="pr_ppn" value="<?= $estimasi_bp['pr_ppn'] ?>"
                            {{ str_contains($vdata['title'], 'Detail') ? 'readonly' : '' }}>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <input type="text" class="form-control form-control-sm" name="user" id="user"
                                value="{{ $estimasi_bp->user }}" readonly>
                        </div>
                        {{-- <div class="vdetail_estimasi_bp"></div> --}}
                    </div>
                @endif
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#pageestimasi').addClass('in show active'); // add active class to the clicked tab
        $("#tabmobil").removeClass('active');
        $("#tabestimasi").addClass('active');
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.error) {
                        ...
                        // document.getElementById("pageestimasi").addClass('active');
                        $('#pagemobil').removeClass(
                            'in show active'); // to remove the current active tab
                        $('#pageestimasi').addClass(
                            'in show active'
    ...
    $('.btnvdetail').click(function(e) {
        var idestimasi = $("#idestimasi").val();
        var noestimasi = $("#noestimasi").val();
        $.ajax({
            type: "get",
            url: 'detailestimasi_bp',
            dataType: "json",
            data: {
                id: idestimasi,
