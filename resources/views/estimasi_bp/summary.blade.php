<div class="card-body">
    <form action="tbmobil_update" method="post" class="formestimasi_bp">
        <div class="row mt-2 mb-2">
            <?php
            $ftotal_jasa = number_format($estimasi_bp->total_jasa, 0, ',', ',');
            $ftotal_part = number_format($estimasi_bp->total_part, 0, ',', ',');
            $ftotal_bahan = number_format($estimasi_bp->total_bahan, 0, ',', ',');
            $ftotal_opl = number_format($estimasi_bp->total_opl, 0, ',', ',');
            $ftotal = number_format($estimasi_bp->total, 0, ',', ',');
            $fdpp = number_format($estimasi_bp->dpp, 0, ',', ',');
            $fpr_ppn = number_format($estimasi_bp->pr_ppn, 0, ',', ',');
            $fppn = number_format($estimasi_bp->ppn, 0, ',', ',');
            $ftotal_estimasi = number_format($estimasi_bp->total_estimasi, 0, ',', ',');
            ?>
            ...
            <div class="col-12 col-sm-6">
                ...
                <input type="text" class="form-control form-control-sm mb-2 text-end" id="grantotal"
                    value="<?= $ftotal_estimasi ?>" style="text-align:right;" readonly>
            </div>
        </div>
    </form>
</div>
