...
<div class="modal-dialog" style="max-width: 50%;">
    <div class="modal-content">
        ...
        @endif --}}
        <form action="{{ $action }}" method="post" class="formtbjnstrmksr">
            @csrf
            @if ($tbjnstrmksr->id)
                @method('put')
            @endif
            <div class="modal-body">
                ...
                <input type="hidden" class="form-control-sm" name="kodelama" id="kodelama"
                    value="{{ $tbjnstrmksr->kode }}">
                <div class="col-md-6 mb-2">
                    <label for="kode" class="form-label mb-1">Kode</label>
                    <input type="text" class="form-control" name="kode" id="kode"
                        value="{{ $tbjnstrmksr->kode }}" {{ str_contains($vdata['title'], 'Detail') ? 'readonly' : '' }}
                ...
                <div class="col-md-12 mb-2">
                    <label for="kodeacc" class="form-label mb-1">Kode Acc</label>
                    <input type="text" class="form-control" name="kodeacc" id="kodeacc"
                        value="{{ $tbjnstrmksr->kodeacc }}"
                        {{ str_contains($vdata['title'], 'Detail') ? 'readonly' : '' }}>
                    @error('kodeacc')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                ...
                <div class="col-md-12 mb-0">
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="aktif" name="aktif"
                            {{ (str_contains($vdata['title'], 'Tambah') ? 'checked' : $tbjnstrmksr->aktif == 'Y') ? 'checked' : '' }}
                ...
                <div class="col-md-12">
                    ...
                    <input type="text" class="form-control" name="user" id="user"
                        value="{{ $tbjnstrmksr->user }}" readonly>
                </div> <?php } ?>
            </div>
...
<script>
    ...
    $(document).ready(function() {
        $('.formtbjnstrmksr').submit(function(e) {
            const form = $(this)
            e.preventDefault();
            $.ajax({
                type: "POST",
                ...
                success: function(response) {
                    ...
                    if (response.error) {
                        ...
                        // .then(function() {
                        //     window.location.href = '/tbjnstrmksr';
                        // });
                        // window.location = '/tbjnstrmksr';
                    }
                },
