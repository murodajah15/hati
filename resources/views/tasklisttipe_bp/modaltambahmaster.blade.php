...
<div class="modal-dialog" style="max-width: 50%;">
    <div class="modal-content">
        ...
        @endif --}}
        <form action="{{ $action }}" method="post" class="formtasklisttipe_bp">
            @csrf
            @if ($tasklisttipe_bp->id)
                @method('put')
            @endif
            <div class="modal-body">
                ...
                <div class="col-md-12 mb-0">
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="aktif" name="aktif"
                            {{ (str_contains($vdata['title'], 'Tambah') ? 'checked' : $tasklisttipe_bp->aktif == 'Y') ? 'checked' : '' }}
                ...
                <div class="col-md-12">
                    ...
                    <input type="text" class="form-control form-control-sm" name="user" id="user"
                        value="{{ $tasklisttipe_bp->user }}" readonly>
                </div> <?php } ?>
            </div>
...
<script>
    ...
    $(document).ready(function() {
        $('.formtasklisttipe_bp').submit(function(e) {
            const form = $(this)
            e.preventDefault();
            $.ajax({
                type: "POST",
                ...
                success: function(response) {
                    ...
                    if (response.error) {
                        ...
                        // });
                        // window.location = '/tasklisttipe_bp';
                    }
                },
