...
@section('content')
    ...
    <div class="content-wrapper">
        ...
        <div class="content-header">
            <div class="container-fluid">
                ...
                <div class="card mt-2">
                    <div class="card-body">
                        <table id="tbl-tasklisttipe_bp"
                            class="table table-bordered table-hover table-sm tbl-tasklisttipe_bp">
                            {{-- <table id="tbl-tasklisttipe_bp" class="table table-bordered table-striped tbl-tasklisttipe_bp"> --}}
                            <thead>
                                <tr>
                                    <th width=30 style="text-align:center;">#</th>
                                    <th width="70">Kode</th>
                                    <th width="350">Nama</th>
                                    <th width="70">Kode Asr.</th>
                                    <th width="350">Nama Asuransi</th>
                                    <th width="10">Aktif</th>
                                    <th width="90">Aksi</th>
                                </tr>
                            </thead>
                            <tbody>
                                {{-- @foreach ($tasklisttipe_bp as $row)
                                    <tr>
    ...
    <script>
        ...
        function reload_table() {
            ...
            $(function() {
                var table = $('.tbl-tasklisttipe_bp').DataTable({
                    ajax: "{{ route('tasklisttipe_bpajax') }}",
        ...
        function reload_table1() {
            $(document).ready(function() {
                $('#tbl-tasklisttipe_bp-data').DataTable({
                    // "destroy": true,
                    "aLengthMenu": [
                        [5, 50, 100, -1],
                        [5, 50, 100, "All"]
            ...
            $.ajax({
                url: "<?= 'tabel_tasklisttipe_bp' ?>",
                beforeSend: function(f) {
                    $('.btnreload').attr('disable', 'disabled')
                    $('.btnreload').html('<i class="fa fa-spin fa-spinner"></i>')
                    // alert('1');
                    $('#tabel_tasklisttipe_bp').html('<center>Loading Table ...</center>');
                },
                success: function(data) {
                    // alert(data);
                    $('#tabel_tasklisttipe_bp').html(data);
        ...
        $('.tomboltambah').click(function(e) {
            e.preventDefault();
            $.ajax({
                url: `{{ route('tasklisttipe_bp.create') }}`,
                dataType: "json",
        ...
        function edit(id) {
            $.ajax({
                type: "get",
                url: `{{ url('tasklisttipe_bp') }}/${id}/edit`,
                dataType: "json",
                success: function(response) {
                    if (response.data) {
