...
if ($rekapitulasi == 'Y') {
  ...
  <th width="60px"><font size="1" color="black">STOCK<br>AKHIR</th>';
    ...
    inner join belid on tbbarang.kode=belid.kdbarang where tbbarang.kode='$kdbarang' and belid.proses='Y' and ";
        $qry .= ($semuaperiode == 'Y' ? "belid.tglbeli>'$tglopname'" : "and (belid.tglbeli>'$tglopname' and belid.tglbeli<='$tgl2')") . ' order by tbbarang.kode';
        ...
        $qry .= ($semuaperiode == 'Y' ? "terimad.tglterima>'$tglopname'" : "and (terimad.tglterima>'$tglopname' and terimad.tglterima<='$tgl2')") . ' order by tbbarang.kode';
        $queryd = mysqli_query($connect, $qry);
        while ($rowd = mysqli_fetch_assoc($queryd)) {
            $nodokumen = $rowd['nodokumen'];
            $tanggal = $rowd['tanggal'];
        ...
        $qry .= ($semuaperiode == 'Y' ? "juald.tgljual>'$tglopname'" : "and (juald.tgljual>'$tglopname' and juald.tgljual<='$tgl2')") . ' order by tbbarang.kode';
        // echo '<br>qry : ' . $qry . '<br>';
        $queryd = mysqli_query($connect, $qry);
        while ($rowd = mysqli_fetch_assoc($queryd)) {
            $nodokumen = $rowd['nodokumen'];
        ...
        $qry .= ($semuaperiode == 'Y' ? "keluard.tglkeluar>'$tglopname'" : "and (keluard.tglkeluar>'$tglopname' and keluard.tglkeluar<='$tgl2')") . ' order by tbbarang.kode';
        // echo '<br>qry : ' . $qry . '<br>';
        $queryd = mysqli_query($connect, $qry);
        while ($rowd = mysqli_fetch_assoc($queryd)) {
            $nodokumen = $rowd['nodokumen'];
