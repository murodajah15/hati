<div class="card-body">
    <form action="tbmobil_update" method="post" class="formwo_gr">
        <div class="row mt-2 mb-2">
            <?php
            $ftotal_jasa = number_format($wo_gr->total_jasa, 0, ',', ',');
            $ftotal_part = number_format($wo_gr->total_part, 0, ',', ',');
            $ftotal_bahan = number_format($wo_gr->total_bahan, 0, ',', ',');
            $ftotal_opl = number_format($wo_gr->total_opl, 0, ',', ',');
            $ftotal = number_format($wo_gr->total, 0, ',', ',');
            $fdpp = number_format($wo_gr->dpp, 0, ',', ',');
            $fpr_ppn = number_format($wo_gr->pr_ppn, 0, ',', ',');
            $fppn = number_format($wo_gr->ppn, 0, ',', ',');
            $ftotal_wo = number_format($wo_gr->total_wo, 0, ',', ',');
            ?>
