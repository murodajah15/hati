...
<div class="modal-dialog" style="max-width: 50%;">
    <div class="modal-content">
        ...
        @endif --}}
        <form action="{{ $action }}" method="post" class="formtbjenix">
            @csrf
            @if ($tbjenis->id)
                @method('put')
            @endif
            <div class="modal-body">
                ...
                <div class="col-md-6 mb-2">
                    ...
                    <input type="text" class="form-control" name="kode" id="kode"
                        value="{{ $tbjenis->kode }}" {{ str_contains($vdata['title'], 'Detail') ? 'readonly' : '' }}
                ...
                <div class="col-md-12 mb-2">
                    ...
                    <input type="text" class="form-control" name="nama" id="nama"
                        value="{{ $tbjenis->nama }}" {{ str_contains($vdata['title'], 'Detail') ? 'readonly' : '' }}>
                    @error('nama')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                ...
                <div class="col-md-12 mb-2">
                    <label for="nama" class="form-label mb-0">Aktif</label><br>
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="aktif" name="aktif"
                            {{ (str_contains($vdata['title'], 'Tambah') ? 'checked' : $tbjenis->aktif == 'Y') ? 'checked' : '' }}
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.error) {
                        ...
                        // .then(function() {
                        //     window.location.href = '/tbjenix';
                        // });
                        // window.location = '/tbjenix';
                    }
                },
