    ...
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" id="staticBackdropLabel">
                {{ $vdata['title'] . ' Detail : ' . $soh_bahan->noso }}
                {{-- @if (isset($vdata))
                    {{ $vdata['title'] }}
                @endif --}}
            </h5>
        ...
        <div class="modal-body">
            <form action="{{ $action }}" method="post" class="formeditdetail">
                ...
                <input type="hidden" class="form-control-sm" name="kdcustomer" id="kdcustomer"
                    value="{{ $sod_bahan->kdcustomer }}">
                <input type="hidden" class="form-control-sm" name="username" id="username"
                    value="{{ $session->get('username') }}">
                <input type="hidden" class="form-control-sm" name="noso" id="noso"
                    value="{{ $soh_bahan->noso }}">
                <input type="hidden" class="form-control-sm" name="nosod" id="nosod"
                    value="{{ $sod_bahan->noso }}">
                <div class='col-md-12'>
                    <table style=font-size:12px; class='table table-striped table table-bordered' width='600px'>
                        <tr>
                            <th>Kode Barang <input type="button" class='btn btn-success btn-sm' value="Clear"
                                    onclick="eraseText()">
                                {{-- <input type="checkbox" class="multiprc" name="multiprc" id="multiprc"> Multi Price --}}
                            </th>
                        ...
                        <td>
                            <div class='input-group'> <input type='text' class='form-control form-control-sm'
                                    ...
                                    size='50' autocomplete='off' value="{{ $sod_bahan->kdbarang }}">
                                ...
                                <span class='input-group-btn'>
                                    &nbsp<button type='button' id='srctb' class='btn btn-success btn-sm'
                                        onclick='caritbbahan()'>TB</button>
                                </span>
                        </td>
                        <input type="hidden" id="kdsatuan" name="kdsatuan" value="{{ $sod_bahan->kdsatuan }}">
                        </td>
                        <td><input type='text' class='form-control form-control-sm' style='width: 20em'
                                id='nmbarang' name='nmbarang' value="{{ $sod_bahan->nmbarang }}" readonly></td>
                        </td>
                        ...
                        <td><input type="text" class='form-control form-control-sm' id='discount' name='discount'
                                ...
                                value="{{ $sod_bahan->discount }}">
                        </td>
                        </td>
                        <td><input type="number" class='form-control form-control-sm' id='subtotal'
                                name='subtotal' style='width: 10em' value="{{ $sod_bahan->subtotal }}" readonly>
                        </td>
                    </table>
            </form>
            <div class="row">
