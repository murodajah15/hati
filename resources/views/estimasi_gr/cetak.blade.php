...
// $queryh = mysqli_query($connect, "select * from estimasi_gr where id='$id' and estimasi_gr.close=1 order by noestimasi");
// $de = mysqli_fetch_assoc($queryh);
// $noestimasi = $de['noestimasi'];
// $tanggal = date('d-M-Y', strtotime($de['tanggal'])); //$de['tgljual']; //

$tanggal = date('d-M-Y', strtotime($estimasi_gr->tanggal)); //$estimasi_gr->tgljual']; //
$kdpemilik = $estimasi_gr->kdpemilik;
$nmpemilik = $estimasi_gr->nmpemilik;
$total = $estimasi_gr->total;
$keluhan = $estimasi_gr->keluhan;
$ppn = $estimasi_gr->pr_ppn;
$rp_ppn = $estimasi_gr->total * ($ppn / 100);
$total_estimasi = $estimasi_gr->total_estimasi;
$pemilik = $estimasi_gr->kdpemilik . '-' . $estimasi_gr->nmpemilik;
...
$html .=
    '<p style="font-family: Tahoma, Verdana, Segoe, sans-serif;font-size:18px;text-align:center;margin-top:1px;margin-bottom:0px;"><u>ESTIMASI GENERAL REPAIR</u></p>
	<table border="0">
    <tr>
      <td colspan=4 style="font-size:18px;font-family: Tahoma, Verdana, Segoe, sans-serif;text-align:center;">&nbsp;&nbsp;NO.' .
    "$estimasi_gr->noestimasi" .
    '</td>
    </tr>
...
$totalppn = 0;
$totaljasa = 0;
foreach ($estimasi_grd_jasa as $row) {
    $n++;
...
// if (count($estimasi_grd_part) > 0 or count($estimasi_grd_bahan) > 0) {
...
$totalppn = 0;
$totalpart = 0;
foreach ($estimasi_grd_part as $row) {
    $n++;
    $fharga = number_format($row['harga'], 0, ',', '.');
    $fqty = number_format($row['qty'], 0, ',', '.');
    $fpr_discount = number_format($row['pr_discount'], 2, ',', '.');
...
foreach ($estimasi_grd_bahan as $row) {
    $n++;
    $fharga = number_format($row['harga'], 0, ',', '.');
    $fqty = number_format($row['qty'], 0, ',', '.');
    $fpr_discount = number_format($row['pr_discount'], 2, ',', '.');
...
$html .=
    ...
    '</b></td></td></tr>';
$html .= '</tbody></table>';
// $fown_risk = number_format($estimasi_gr->own_risk, 0, ',', '.');
// $html .= '<br>Own Risk : Rp. ' . $fown_risk . ',-<br>';
$html .= '<font size=2><br>Jakarta, ' . date('d', strtotime($estimasi_gr->tanggal)) . ' ' . date('M', strtotime($estimasi_gr->tanggal)) . ' ' . date('Y', strtotime($estimasi_gr->tanggal)) . ',';
$html .=
