<div class="card-body">
    <form action="tbmobil_update" method="post" class="formestimasi_gr">
        <div class="row mt-2 mb-2">
            <?php
            $ftotal_jasa = number_format($estimasi_gr->total_jasa, 0, ',', ',');
            $ftotal_part = number_format($estimasi_gr->total_part, 0, ',', ',');
            $ftotal_bahan = number_format($estimasi_gr->total_bahan, 0, ',', ',');
            $ftotal_opl = number_format($estimasi_gr->total_opl, 0, ',', ',');
            $ftotal = number_format($estimasi_gr->total, 0, ',', ',');
            $fdpp = number_format($estimasi_gr->dpp, 0, ',', ',');
            $fpr_ppn = number_format($estimasi_gr->pr_ppn, 0, ',', ',');
            $fppn = number_format($estimasi_gr->ppn, 0, ',', ',');
            $ftotal_estimasi = number_format($estimasi_gr->total_estimasi, 0, ',', ',');
            ?>
