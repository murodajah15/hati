    ...
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" id="staticBackdropLabel">
                {{ $vdata['title'] . ' : ' . $tasklisttipe_bp['kode'] . '-' . $tasklisttipe_bp['nama'] . '-' . $tasklisttipe_bp['kdasuransi'] . '-' . $tasklisttipe_bp['nmasuransi'] }}
                {{-- @if (isset($vdata))
                    {{ $vdata['title'] }}
                @endif --}}
            </h5>
        ...
        <div class="modal-body">
                                ...
                                <div class="col-12 col-sm-12">
                                    <div class="input-group mb-1">
                                        ...
                                        &nbsp;&nbsp;<button class="btn btn-primary btn-sm" type="button"
                                            id="proses_salin_tasklist_bpd">Proses</button>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="col-md-12">
                <form action="simpantasklisttipe_bpd" method="post" class="formtasklisttipe_bpd">
                    @csrf
            ...
            <div class="card mt-0">
                <div class="card-body">
                    ...
                    {{-- <div class='col-md-12'> --}}
                    <table id='tbl-tasklisttipe_bpd' style='font-size:13px;'
...
<script>
    ...
    function reload_table_tasklisttipe_bpd() {
        $(function() {
            var vkdtasklist = $("#kdtasklist").val();
            var vkdasuransi = $("#kdasuransi").val();
            var table = $('#tbl-tasklisttipe_bpd').DataTable({
                ajax: "{{ url('tasklisttipe_bpdajax') }}?kdtasklist=" + vkdtasklist + "&kdasuransi=" +
                    vkdasuransi,
                type: "GET",
                scrollY: '42vh',
                // info: true,
                ...
                columns: [{
                    ...
                    },
                    {
                        data: null,
                        render: function(data, type, row, meta) {
                            return `<a href="#${row.id}"><button onclick="edittasklisttipe_bpd(${row.id})" class='btn btn-sm btn-warning' href='javascript:void(0)' <?= $edit == 1 ? '' : 'disabled' ?>><i class='fa fa-edit'></i></button></a>
    ...
    $('#proses_salin_tasklist_bpd').click(function(e) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.sukses == false) {
                                ...
                                // .then(function() {
                                //     window.location.href = '/tasklisttipe_bp';
                                // });
                            }
                        },
                        error: function(xhr, ajaxOptions, thrownError) {
    ...
    function edittasklisttipe_bpd(id) {
        $.ajax({
            type: "get",
            // url: `{{ url('tasklisttipe_bp_detail') }}/${id}`,
            url: `{{ url('edittasklisttipe_bpd') }}`,
            dataType: "json",
            data: {
                id: id,
                _method: "GET",
