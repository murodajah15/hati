...
// $queryh = mysqli_query($connect, "select * from no_bp where id='$id' and no_bp.close=1 order by nowo");
// $de = mysqli_fetch_assoc($queryh);
// $nowo = $de['nowo'];
// $tanggal = date('d-M-Y', strtotime($de['tanggal'])); //$de['tgljual']; //

$tanggal = date('d-M-Y', strtotime($wo_bp->tanggal)); //$wo_bp->tgljual']; //
$kdpemilik = $wo_bp->kdpemilik;
$nmpemilik = $wo_bp->nmpemilik;
$total = $wo_bp->total;
$keluhan = $wo_bp->keluhan;
$ppn = $wo_bp->pr_ppn;
$rp_ppn = $wo_bp->total * ($ppn / 100);
$total_no = $wo_bp->total_no;
$pemilik = $wo_bp->kdpemilik . '-' . $wo_bp->nmpemilik;
...
$html .=
    '<p style="font-family: Tahoma, Verdana, Segoe, sans-serif;font-size:18px;text-align:center;margin-top:1px;margin-bottom:0px;"><u>WORK ORDER BODY REPAIR</u></p>
	<table border="0">
    <tr>
...
// if (count($wo_bpd_part) > 0 or count($wo_bpd_bahan) > 0) {
...
$html .=
    ...
    '</b></td></td></tr>';
$html .= '</tbody></table>';
$fown_risk = number_format($wo_bp->own_risk, 0, ',', '.');
$html .= '<br>Own Risk : Rp. ' . $fown_risk . ',-<br>';
$html .= '<font size=2><br>Jakarta, ' . date('d', strtotime($wo_bp->tanggal)) . ' ' . date('M', strtotime($wo_bp->tanggal)) . ' ' . date('Y', strtotime($wo_bp->tanggal)) . ',';
$html .=
