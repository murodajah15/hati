...
<div class="modal-dialog" style="max-width: 50%;">
    <div class="modal-content">
        ...
        @endif --}}
        <form action="{{ $action }}" method="post" class="formtbtipe">
            @csrf
            @if ($tbtipe->id)
                @method('put')
            @endif
            <div class="modal-body">
                ...
                <div class="col-md-6 mb-2">
                    ...
                    <input type="text" class="form-control" name="kode" id="kode" value="{{ $tbtipe->kode }}"
                ...
                <div class="col-md-12 mb-2">
                    ...
                    <input type="text" class="form-control" name="nama" id="nama" value="{{ $tbtipe->nama }}"
                        {{ str_contains($vdata['title'], 'Detail') ? 'readonly' : '' }}>
                    @error('nama')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                ...
                <div class="col-md-12">
                    <div class="input-group mb-0">
                        <label for="kdmodel" class="form-label mb-1">Model</label><br>
                        <input type="hidden" class="form-control" name="kdmodele" id="kdmodele"
                            value="<?= $tbtipe['kdmodel'] ?>">
                    </div>
                    <div class="input-group mb-2">
                        <select class="form-control" name='kdmodel' id="kdmodel"
                ...
                <div class="col-md-12 mb-0">
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="aktif" name="aktif"
                            {{ (str_contains($vdata['title'], 'Tambah') ? 'checked' : $tbtipe->aktif == 'Y') ? 'checked' : '' }}
                            {{ str_contains($vdata['title'], 'Detail') ? 'disabled' : '' }}>
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.error) {
                        ...
                        // .then(function() {
                        //     window.location.href = '/tbtipe';
                        // });
                        // window.location = '/tbtipe';
                    }
                },
