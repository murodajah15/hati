...
@section('content')
    ...
    <script>
        ...
        function cetak(id) {
            $url = "{{ url('po_bahancetak') }}?id=" + id
            window.open($url, '_blank')
        }

        // Nampilin list data pilihan ===================
        function caritbbahan() {
            $.ajax({
                method: "GET",
                url: "<?= url('caritbbahan') ?>",
                dataType: "json",
        ...
        function tgljttempo() {
            ...
            // conpole.log(y + '-' + (m <= 9 ? '0' + m : m) + '-' + (d <= 9 ? '0' + d : d));
