...
<div class="modal-dialog" style="max-width: 50%;">
    <div class="modal-content">
        ...
        @endif --}}
        <form action="{{ $action }}" method="post" class="formtbmodel">
            @csrf
            @if ($tbmodel->id)
                @method('put')
            @endif
            <div class="modal-body">
                ...
                <div class="col-md-6 mb-2">
                    ...
                    <input type="text" class="form-control" name="kode" id="kode"
                        value="{{ $tbmodel->kode }}" {{ str_contains($vdata['title'], 'Detail') ? 'readonly' : '' }}
                ...
                <div class="col-md-12 mb-2">
                    ...
                    <input type="text" class="form-control" name="nama" id="nama"
                        value="{{ $tbmodel->nama }}" {{ str_contains($vdata['title'], 'Detail') ? 'readonly' : '' }}>
                    @error('nama')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                ...
                <div class="col-md-12">
                    <div class="input-group mb-0">
                        <label for="kdmerek" class="form-label mb-1">Merek</label><br>
                        <input type="hidden" class="form-control" name="kdmereke" id="kdmereke"
                            value="<?= $tbmodel['kdmerek'] ?>">
                    </div>
                    <div class="input-group mb-2">
                        <select class="form-control" name='kdmerek' id="kdmerek"
                ...
                <div class="col-md-12 mb-2">
                    <div class="input-group mb-0">
                        <label for="nama" class="form-label mb-0">Aktif</label>
                        <div class="form-check form-switch">
                            &nbsp;&nbsp;<input class="form-check-input" type="checkbox" id="aktif" name="aktif"
                                {{ (str_contains($vdata['title'], 'Tambah') ? 'checked' : $tbmodel->aktif == 'Y') ? 'checked' : '' }}
                                {{ str_contains($vdata['title'], 'Detail') ? 'disabled' : '' }}>
                        </div>
                    </div>
                </div>
...
<script>
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.error) {
                        ...
                        // .then(function() {
                        //     window.location.href = '/tbmodel';
                        // });
                        // window.location = '/tbmodel';
                    }
                },
    ...
    var carikdmerek = $("#kdmereke").val();
    // if (carikdmerek != "") {
    $.ajax({
        url: "<?= url('ambildatatbmerek') ?>",
        dataType: "json",
        data: {
            'kdmerek': carikdmerek
        },
