    ...
    <table id="tabel-teman" class="table table-striped" style="width:100%">
      <thead>
        <tr>
          ...
          <th scope="col" width="130">Aksi</th>
        </tr>
      </thead>
      <tbody>
      </tbody>
    ...
    $('#tabel-teman').DataTable({
      ...
      columns: [{
        ...
        },
        {
          data: null,
          render: function(data, type, row, meta) {
            //   return `<button class='btn btn-sm btn-secondary' onclick="detail(${row.id})"><i class='fa fa-eye'></i></button>
            //   <button class='btn btn-sm btn-secondary' onclick="edit(${row.id})"><i class='fa fa-eye'></i></button>
            ...
            <a href="#${row.id}" onclick="deleteteman(${row.id})"><button class='btn btn-sm btn-danger' href='javascript:void(0)'><i class='fa fa-trash'></i></button></a>`;
          }
