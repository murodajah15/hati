...
<div class="row">
  <div class="container-fluid mt-2">
    ...
    <div class="container mt-2">
      ...
      <!-- <table class="table table-bordered table-striped" id="tbl-mekanik-data"> -->
      <table id="tbl-mekanik-data" class="table table-striped" style="width:100%">
        <!-- <table class="table"> -->
        <thead>
          <tr>
            <th width="10">#</th>
            <th width="80">Kode</th>
            <th width="80">Kode H3S</th>
...
<script>
  $(document).ready(function() {
    $('#tbl-mekanik-data').DataTable({
      ...
      ajax: {
        url: '<?= site_url('/tbmekanik/ajax-load-data') ?>',
        type: 'POST',
      },
      columns: [{
          data: null,
        ...
        // },
        {
          ...
          render: function(data, type, row, meta) {
            ...
            return `<a href="#${row.id}"><button class='btn btn-sm btn-warning' href='javascript:void(0)' onclick="edit(${row.id})" <?= $edit == 1 ?  '' : '' ?>><i class='fa fa-edit'></i></button></a>
  ...
  $('.tomboltambah').click(function(e) {
    e.preventDefault();
    $.ajax({
      url: "<?= site_url('tbmekanik/formtambah') ?>",
      dataType: "json",
      success: function(response) {
        $('.viewmodaltambah').html(response.data).show();
        $('#modaltambahmekanik').modal('show');
      },
