...
<script>
  ...
  function reload_table() {
    ...
    //   $('#tbl-klpuser-data').DataTable();
    // });

    $.ajax({
      url: "<?= site_url('tbklpuser/table_klpuser'); ?>",
      beforeSend: function(f) {
        $('.btnreload').attr('disable', 'disabled')
        $('.btnreload').html('<i class="fa fa-spin fa-spinner"></i>')
        // alert('1');
        $('#tabel_klpuser').html('<center>Loading Table ...</center>');
      },
      success: function(data) {
        // alert(data);
        $('#tabel_klpuser').html(data);
  ...
  $('.tomboltambah').click(function(e) {
    e.preventDefault();
    $.ajax({
      url: "<?= site_url('tbklpuser/formtambah') ?>",
      dataType: "json",
    ...
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          $href = "/tbklpuser/";
          $.ajax({
            url: "<?php echo site_url('tbklpuser/urutkan') ?>",
            type: "POST",
            dataType: "JSON",
  ...
  function detail($id) {
    $.ajax({
      type: "post",
      url: "<?= site_url('tbklpuser/formdetail') ?>",
      dataType: "json",
      data: {
        id: $id
      },
  ...
  function edit($id) {
    $.ajax({
      type: "post",
      url: "<?= site_url('tbklpuser/formedit') ?>",
      dataType: "json",
      data: {
        id: $id
      },
    ...
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          $href = "/tbklpuser/";
          $.ajax({
            url: "<?php echo site_url('tbklpuser/hapus') ?>/" + $id,
            type: "POST",
            dataType: "JSON",
            success: function(data1) {
              //if success reload ajax table
