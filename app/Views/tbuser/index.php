...
<script>
  ...
  function detail($id) {
    $.ajax({
      type: "post",
      url: "<?= site_url('tbuser/formdetail') ?>",
      dataType: "json",
      data: {
        id: $id
      },
    ...
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          $href = "/tbuser/";
          $.ajax({
            url: "<?php echo site_url('tbuser/hapus') ?>/" + $id,
            type: "POST",
            dataType: "JSON",
