...
<script>
  ...
  function reload_table_wo_bp() {
    $('#tbl-wo-bp').DataTable({
      ...
      ajax: {
        url: '<?= site_url('/bahan_bp/ajax-load-data') ?>',
        type: 'POST',
      },
      columns: [{
          data: null,
        ...
        },
        {
          ...
          name: 'total_bahan',
          render: function(data, type, row, meta) {
            return meta.settings.fnFormatNumber(row.total_bahan);
          }
        },
        {
          data: null,
          render: function(data, type, row, meta) {
            if (row['batal'] == 1) {
              ...
              <ul class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                <li><a class="dropdown-item disabled" onclick="close_bahan_bp(${row.id})" href="#" readonly><i class='fa fa-arrow-right'"></i> Close Bahan</a></li>
                <li><a class="dropdown-item <?= $unproses == 1 ?  '' : 'disabled' ?>"" onclick="unclose_bahan_bp(${row.id})" href="#" readonly><i class='fa fa-arrow-left'"></i> Unclose Bahan</a></li>
              </ul>
            </div>`;
              } else {
                return `<div class="btn-group" role="group">
              ...
              <ul class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                <li><a class="dropdown-item <?= $edit == 1 ?  '' : 'disabled' ?>" onclick="input_bahan_bp(${row.id})" href="#" readonly><i class='fa fa-book'"></i> Pembebanan Bahan</a></li>
                <li><a class="dropdown-item <?= $proses == 1 ?  '' : 'disabled' ?>" onclick="close_bahan_bp(${row.id})" href="#" readonly><i class='fa fa-arrow-right'"></i> Close Bahan</a></li>
                <li><a class="dropdown-item disabled" onclick="unclose_bahan_bp(${row.id})" href="#" readonly><i class='fa fa-arrow-left'"></i> Unclose Bahan</a></li>
              </ul>
            </div>`;
              }
            }
  ...
  function input_bahan_bp($id) {
    $.ajax({
      type: "post",
      url: "<?= site_url('bahan_bp/input_bahan_bp') ?>",
      dataType: "json",
      data: {
        id: $id
      },
  ...
  function close_bahan_bp($id) {
    swal({
      ...
      .then((willcetak) => {
        if (willcetak) {
          $.ajax({
            url: "<?= site_url('bahan_bp/close_bahan_bp') ?>",
            type: "POST",
            data: {
              id: $id
            },
  ...
  function unclose_bahan_bp($id) {
    swal({
      ...
      .then((willcetak) => {
        if (willcetak) {
          $.ajax({
            url: "<?= site_url('bahan_bp/unclose_bahan_bp') ?>",
            type: "POST",
            data: {
              id: $id
            },
            success: function(response) {
              if (response.error) {
                ...
                if (data_response['sukses'] == 'Data berhasil disimpan') {
                  ...
                  swal({
                    title: "Data gagal di unclose (WO sudah close) ! ",
                    text: "",
                    icon: "error"
                  })
                  reload_table_wo_bp();
