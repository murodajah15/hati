...
<div class="row">
  ...
  <div class="container mt-1">
    <table id="tbl-warna-data" class="table table-striped" style="width:100%">
      <thead>
        <tr>
          <th width="30">No.</th>
          <th>Kode</th>
          ...
          foreach ($tbwarna as $k) : ?>
            <tr>
              ...
              <td width="100">
                ...
                ?>
                <!-- <button type='Button' class='btn btn-danger btn-sm' onClick="hapus(`<?= $k->id ?>`, `<?= $k->kode ?>`)"><i class="fa fa-trash"></i></button> -->
                <?php if ($hapus == 1) {
                ?>
                  <button type='Button' class='btn btn-danger btn-sm' onClick="hapus(`<?= $k->id ?>`, `<?= $k->kode ?>`)"><i class="fa fa-trash"></i></button>
                <?php
                ...
                ?>
                  <button type='Button' class='btn btn-danger btn-sm' onClick="hapus(`<?= $k->id ?>`, `<?= $k->kode ?>`)" disabled><i class="fa fa-trash"></i></button>
                <?php
                }
                ?>
                <!-- </form> -->
