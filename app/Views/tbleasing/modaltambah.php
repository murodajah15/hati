<div class="modal fade" role="dialog" id="modaltambahleasing" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog" style="max-width: 90%;">
    <div class="modal-content">
      ...
      <!-- <?= form_open('tbleasing/simpandata', ['class' => 'formtbleasing']) ?> -->
      <form action='tbleasing/simpandata' method='post' enctype='multipart/form-data' class='formtbleasing'>
        <div class="modal-body">
          <div class="row mb-2">
            <div class="col-12 col-sm-6">
              <?php
              $kode = 'L' . date('Y') . date('m') . sprintf("%05s", intval(substr('00000', -5)) + 1);
              foreach ($kodeleasing as $row) {
                if ($row->kode <> null) {
                  $kode = 'L' . date('Y') . date('m') . sprintf("%05s", intval(substr($row->kode, -5)) + 1);
                } else {
                  $kode = 'L' . date('Y') . date('m') . sprintf("%05s", intval(substr('0', -5)) + 1);
                }
              }
              ?>
...
<script>
  var myModal = document.getElementById('modaltambahleasing')
