          ...
          <div class="col-lg-12 margin-tb">
            <table id="example" class="table table-bordered table-striped" style="width:100%">
              ...
              <tbody id="isi_data" class="isi_data">
                <?php //$no=1+(10*($page-1));
                $no = 1;
                foreach ($tbjasap as $row) : ?>
                  <tr id="tr" style="cursor: pointer; ">
                    <td><?= $no; ?></td>
                    <td id="td" data-bs-dismiss="modal" onclick="post_data_tbjasa();"><?= $row['kode']; ?></td>
                    <td id="td" data-bs-dismiss="modal" onclick="post_data_tbjasa();"><?= $row['nama']; ?></td>
                  </tr>
                <?php $no++;
                endforeach; ?>
              </tbody>
  ...
  <script>
    ...
    function post_data_tbjasa() {
      ...
      tbody.onclick = function(e) {
        ...
        $('#parent_id').val(((dt_split[1]).replace("--separator--", "")).trim());
        $('#parent_name').val(((dt_split[2]).replace("--separator--", "")).trim());
      };
    }
  </script>