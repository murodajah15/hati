...
<div class="modal fade" id="modaledit" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog" style="max-width: 50%;">
    <div class="modal-content">
      ...
      <?= form_open('tbtipe/updatedata', ['class' => 'formtbtipe']) ?>
      <?= csrf_field(); ?>
      <input type="hidden" class="form-control" name="id" id="id" value="<?= $tbtipe['id'] ?>">
      <input type="hidden" class="form-control" name="kodelama" id="kodelama" value="<?= $tbtipe['kode'] ?>">
      <div class="modal-body">
        <label for="nama" class="form-label mb-1">Model</label>
        <select class="form-select" name="kdmodel" id="kdmodel">
          <option value="">[Pilih Model]
            <?php
            foreach ($tbmodel as $key) {
              if ($key['kode'] == $tbtipe['kdmodel']) {
