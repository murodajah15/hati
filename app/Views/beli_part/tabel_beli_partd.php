...
<table class="table table-striped tbl_beli_partd" style="width:100%">
  <thead>
    <tr>
      <th width="30">#</th>
  ...
  <tbody>
    ...
    ?>
      <tr>
        ...
        if ($nmform == "detail") {
        ?>
          <td><button class="btn btn-outline-danger btn-sm" onClick="hapus_beli_partd(`<?= $r['id'] ?>`)" type="button" id="carijasa" disabled><i class="fa fa-trash"></i></button></td>
        <?php
        } else {
        ?>
          <td><button class="btn btn-outline-warning btn-sm" onClick="tampil_detail_beli_partd(`<?= $r['id'] ?>`)" type="button"><i class="fa fa-edit"></i></button>
            <button class="btn btn-outline-danger btn-sm" onClick="hapus_beli_partd(`<?= $r['id'] ?>`)" type="button" id="carijasa"><i class="fa fa-trash"></i></button>
          </td>
        <?php
        }
        ?>
...
<script>
  $(document).ready(function() {
    $('.tbl_beli_partd').DataTable();
  });

  function tampil_detail_beli_partd($id) {
    $.ajax({
      url: "<?php echo site_url('beli_part/tampil_detail_beli_partd') ?>/" + $id,
      type: "POST",
      data: {
        id: $id
      },
