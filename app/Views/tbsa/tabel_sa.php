...
<div class="row">
  ...
  <div class="container mt-1">
    <!-- <table class="table table-bordered table-striped" id="tbl-sa-data"> -->
    <table id="tbl-sa-data" class="table table-striped" style="width:100%">
      <!-- <table class="table"> -->
      <thead>
        <tr>
...
<script>
  $(document).ready(function() {
    $('#tbl-sa-data').DataTable({
      ...
      ajax: {
        url: '<?= site_url('/tbsa/ajax-load-data') ?>',
        type: 'POST',
      },
      columns: [{
          data: null,
        ...
        },
        {
          ...
          data: null,
          render: function(data, type, row, meta) {
            return `<a href="#" onclick="detail(${row.id})">${row.kdsa}</a>`;
          }
        },
        {
          data: 'nama',
        ...
        },
        {
          ...
          render: function(data, type, row, meta) {
            ...
            <a href="#${row.id},${row.kdsa}"><button onclick="hapus(${row.id})" class='btn btn-sm btn-danger' href='javascript:void(0)' <?= $hapus == 1 ?  '' : 'disabled' ?>><i class='fa fa-trash'></i></button></a>`;
          }
        },
      ],
      // columnDefs: [{
