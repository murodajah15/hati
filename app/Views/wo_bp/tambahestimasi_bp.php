...
<div class="modal fade" id="tambahestimasi_bp" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog" style="max-width: 90%;">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <?php if ($tambah == 1) {
        ?>
          <?= form_open('estimasi_bp/simpanestimasi_bp', ['class' => 'formtambahestimasi_bp']) ?>
          ...
          <div class="row mb-2 mt-4">
            <div class="col-12 col-sm-6">
              ...
              ?>
              <label for="nama" class="form-label mb-1">No. estimasi_bp / Tanggal (M-D-Y)</label>
              <div class="input-group mb-1">
                <input type='text' class='form-control form-control-sm mb-2' value="AUTO GENERATE" id="noestimasi_bpjadi" name="noestimasi_bpjadi" readonly style="width: 5%">
                <input type="datetime-local" class='form-control form-control-sm mb-2' name='tanggal' id='tanggal' value="<?= $tglestimasi_bp ?>" style="width: 40%" readonly>
              </div>
          ...
          <div class="col-md-12">
            <button type="submit" value="estimasi_bp" class="btn btn-flat btn-primary btnsimpanestimasi_bp" id="btnsimpanestimasi_bp"><i class="fa fa-file"></i> Simpan</button>
    ...
    ?>
      ...
      <?php
            ...
            echo "<p>Anda tidak berhak membuat estimasi_bp / WO</p>";
          }
      ?>
...
<script>
  $(document).ready(function() {
    $('.formtambahestimasi_bp').on('click', '.btnsimpanestimasi_bp', function(e) {
      $('.formtambahestimasi_bp').submit(function(e) {
        e.preventDefault();
        $.ajax({
          url: $(this).attr('action'),
          // url: "<?= site_url('estimasi_bp/simpanestimasi_bp') ?>",
          type: "post",
          data: $(this).serialize(),
          dataType: "json",
          beforeSend: function() {
            $('.btnsimpanestimasi_bp').attr('disable', 'disabled')
            $('.btnsimpanestimasi_bp').prop('disable', false)
            $('.btnsimpanestimasi_bp').html('<i class="fa fa-spin fa-spinner"></i>')
          },
          complete: function() {
            $('.btnsimpanestimasi_bp').removeAttr('disable')
            $('.btnsimpanestimasi_bp').html('<i class="fa fa-file"></i> Simpan estimasi_bp')
            $('.btnsimpanestimasi_bp').prop('disabled', true)
          },
          success: function(response) {
            if (response.error) {
              ...
              $('#tambahestimasi_bp').modal('hide');
              let hasil;
              hasil = document.getElementById("title").value;
              swal({
                title: "estimasi_bp Berhasil disimpan ",
                text: hasil,
                icon: "success",
              })
            }
  ...
  $("#tambahestimasi_bp").on('hide.bs.modal', function() {
    // alert('The modal is about to be hidden.');
    // reload_table_estimasi_bp()
    // $('#modalestimasi_bp b:first').tab('show');
    $('#modalestimasi_bp').modal('show');
    $('#tabestimasi_bp').tab('show');
    $('#tabestimasi_bp').click();
  });
</script>