...
<div class="modal fade" id="modalestimasi_bp" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <ul class="nav nav-tabs">
          <li class="nav-item">
            <a href="#data" class="nav-link active" data-bs-toggle="tab" id="kendaraan">Data Kendaraan / + Estimasi</a>
          </li>
          <li class="nav-item">
            <a href="#datawo" class="nav-link" data-bs-toggle="tab" id="tabestimasi_bp">Estimasi WO Body Repair&nbsp <button class="btn btn-flat btn-info btn-sm mb-2 btn-float-right btnreload" type="button"><i class="fa fa-spinner"></i></button></a>
          </li>
          <!-- <li class="nav-item">
        ...
        <div class="tab-content">
          <div class="tab-pane fade show active" id="data">
            ...
            <div class="modal-footer">
              ...
              ?>
                ...
                <button type="button" id="btnestimasi_bp" class="btn btn-primary btnestimasi_bp" disabled>Tambah Estimasi Body Repair</button>
              <?php
              } else {
              ?>
          ...
          <div class="tab-pane fade datawo" id="datawo">
            Estimasi
            <div id="tabel_estimasi_bp"></div>
            WO
            <div id="tabel_wo_bp"></div>
          </div>
        </div>
      </div>
      <div class="modal-footer">
...
<script>
  ...
  // var isModelOpen = $('#datawo').hasClass('in');
  // if (isModelOpen) {
  //   alert("Modal is Open");
  // } else {
  ...
  $("#tabestimasi_bp").on("click", function() {
    reload_table_wo_bp();
    reload_table_estimasi_bp();
  });
  ...
  function reload_table_wo_bp() {
    ...
    $.ajax({
      ...
      beforeSend: function(f) {
        ...
        // alert('1');
        $('#tabel_wo_bp').html('<center>Loading Table ...</center>');
      },
      ...
      $.ajax({
        ...
        success: function(response) {
          if (response.error) {
            ...
            document.getElementById("btnestimasi_bp").disabled = false;
            // document.getElementById("btnwo").disabled = false;
            // document.getElementById("tabestimasi_bp").disabled = false;
            swal({
  ...
  $('#btnestimasi_bp').click(function() {
    $nopolisi = document.getElementById('nopolisi').value;
    $.ajax({
      url: "<?= site_url('estimasi_bp/tambahestimasi_bp') ?>",
      dataType: "json",
      type: "post",
      data: {
        nopolisi: $nopolisi
      ...
      success: function(response) {
        $('.viewmodalestimasi_bp').html(response.data).show();
        $('#tambahestimasi_bp').modal('show');
      },
      error: function(xhr, ajaxOptions, thrownError) {
        alert(xhr.status + "\n" + xhr.responseText + "\n" + thrownError);
      }
