...
<script>
  ...
  function reload_table() {
    ...
    $.ajax({
      url: "<?= site_url('tbdisc/table_disc'); ?>",
      beforeSend: function(f) {
        $('.btnreload').attr('disable', 'disabled')
        $('.btnreload').html('<i class="fa fa-spin fa-spinner"></i>')
        // alert('1');
        $('#tabel_disc').html('<center>Loading Table ...</center>');
      },
      success: function(data) {
        // alert(data);
        $('#tabel_disc').html(data);
  ...
  function detail($id) {
    $.ajax({
      type: "post",
      url: "<?= site_url('tbdisc/formdetail') ?>",
      dataType: "json",
      data: {
        id: $id
      },
    ...
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          $href = "/tbdisc/";
          $.ajax({
            url: "<?php echo site_url('tbdisc/hapus') ?>/" + $id,
            type: "POST",
            dataType: "JSON",
            success: function(data) {
