...
<script>
  ...
  function reload_table() {
    ...
    $.ajax({
      url: "<?= site_url('saplikasi/table_saplikasi'); ?>",
      beforeSend: function(f) {
        $('.btnreload').attr('disable', 'disabled')
        $('.btnreload').html('<i class="fa fa-spin fa-spinner"></i>')
        // alert('1');
        $('#tabel_saplikasi').html('<center>Loading Table ...</center>');
      },
      success: function(data) {
        // alert(data);
        $('#tabel_saplikasi').html(data);
  ...
  $('.tomboltambah').click(function(e) {
    e.preventDefault();
    $.ajax({
      url: "<?= site_url('saplikasi/formtambah') ?>",
      dataType: "json",
  ...
  function detail($id) {
    $.ajax({
      type: "post",
      url: "<?= site_url('saplikasi/formdetail') ?>",
      dataType: "json",
      data: {
        id: $id
      },
  ...
  function edit($id) {
    $.ajax({
      type: "post",
      url: "<?= site_url('saplikasi/formedit') ?>",
      dataType: "json",
      data: {
        id: $id
      },
    ...
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          $href = "/saplikasi/";
          $.ajax({
            url: "<?php echo site_url('saplikasi/hapus') ?>/" + $id,
            type: "POST",
            dataType: "JSON",
