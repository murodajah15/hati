  ...
  <div class="container-fluid px-4">
    ...
    <div class="card mb-4">
      ...
      <div class="card-body">
        ...
        ?>
          <button class="btn btn-flat btn-primary btn-sm mb-2 tomboltambah" type="button" disabled><i class="fa fa-plus"></i> </button>
        <?php
        }
        ?>
        <div id="tabel_merek"></div>
      </div>
    </div>
</main>
...
<script>
  ...
  function reload_table() {
    ...
    $(document).ready(function() {
      $('#tbl-merek-data').DataTable();
    });

    $.ajax({
      url: "<?= site_url('tbmerek/table_merek'); ?>",
      beforeSend: function(f) {
        $('.btnreload').attr('disable', 'disabled')
        $('.btnreload').html('<i class="fa fa-spin fa-spinner"></i>')
        // alert('1');
        $('#tabel_merek').html('<center>Loading Table ...</center>');
      },
      success: function(data) {
        // alert(data);
        $('#tabel_merek').html(data);
  ...
  $('.tomboltambah').click(function(e) {
    e.preventDefault();
    $.ajax({
      url: "<?= site_url('tbmerek/formtambah') ?>",
      dataType: "json",
  ...
  function detail($id) {
    $.ajax({
      type: "post",
      url: "<?= site_url('tbmerek/formdetail') ?>",
      dataType: "json",
      data: {
        id: $id
      },
    ...
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          $href = "/tbmerek/";
          $.ajax({
            url: "<?php echo site_url('tbmerek/hapus') ?>/" + $id,
            type: "POST",
            dataType: "JSON",
            success: function(data) {
