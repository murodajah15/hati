...
<div class="viewmodalhisuser" style="display: none;"></div>
...
<script>
  ...
  function reload_table() {
    ...
    ?>
    $(document).ready(function() {
      $('#tbl-hisuser-data').DataTable();
    });

    $.ajax({
      url: "<?= site_url('hisuser/table_hisuser'); ?>",
      beforeSend: function(f) {
        $('.btnreload').attr('disable', 'disabled')
        $('.btnreload').html('<i class="fa fa-spin fa-spinner"></i>')
        // alert('1');
        $('#tabel_hisuser').html('<center>Loading Table ...</center>');
      },
      success: function(data) {
        // alert(data);
        $('#tabel_hisuser').html(data);
  ...
  $('.tomboltambah').click(function(e) {
    e.preventDefault();
    $.ajax({
      url: "<?= site_url('hisuser/formtambah') ?>",
      dataType: "json",
  ...
  function detail($id) {
    $.ajax({
      type: "post",
      url: "<?= site_url('hisuser/modaldetail') ?>",
      dataType: "json",
      data: {
        id: $id
      },
  ...
  function detailhisuser($id) {
    $.ajax({
      type: "get",
      url: "<?= site_url('wo/detailhisuser') ?>",
      dataType: "json",
      data: {
        id: $id
      },
      success: function(response) {
        if (response.sukses) {
          $('.viewmodal').html(response.sukses).show();
          $('#modaldetail_hisuser').modal('show');
        }
      },
      error: function(xhr, ajaxOptions, thrownError) {
        alert(xhr.status + "\n" + xhr.responseText + "\n" + thrownError);
    ...
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          $href = "/wo/";
          $.ajax({
            url: "<?php echo site_url('hisuser/hapus') ?>/" + $id,
            type: "POST",
            dataType: "JSON",
            success: function(data) {
  ...
  function detail_hisuser($id) {
    $.ajax({
      type: "post",
      url: "<?= site_url('hisuser/formdetail') ?>",
      dataType: "json",
      data: {
        id: $id
      },
  ...
  function edit_hisuser($id) {
    $.ajax({
      type: "post",
      url: "<?= site_url('hisuser/formedit') ?>",
      dataType: "json",
      data: {
        id: $id
      },
  ...
  function formdetailhisuser($id) {
    $.ajax({
      type: "post",
      url: "<?= site_url('wo/formdetailhisuser') ?>",
      dataType: "json",
      data: {
        id: $id
      },
  ...
  function modalhisuser($id) {
    $.ajax({
      type: "POST",
      url: "<?= site_url('hisuser/modalhisuser') ?>",
      dataType: "json",
      data: {
        id: $id,
      },
  ...
  function unproses_hisuser($id) {
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          $.ajax({
            url: "<?php echo site_url('hisuser/unproses') ?>/" + $id,
            data: {
              id: $id,
            },
            type: "POST",
  ...
  function cetak_hisuser($id) {
    ...
    //   .then((willcetak) => {
    //     if (willcetak) {
    var w = window.open('hisuser/cetakhisuser/' + $id);
    $.ajax({
      url: "", //<?php echo site_url('hisuser/cetakhisuser') ?>",
      // url: "<?php echo site_url('hisuser/cetakhisuser') ?>",
      // type: "POST",
      data: {
        id: $id
