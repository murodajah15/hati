...
<script>
  ...
  function reload_table_wo_bp() {
    $('#tbl-wo-bp').DataTable({
      ...
      ajax: {
        url: '<?= site_url('/part_bp/ajax-load-data') ?>',
        type: 'POST',
      },
      columns: [{
          data: null,
        ...
        },
        {
          ...
          name: 'total_part',
          render: function(data, type, row, meta) {
            return meta.settings.fnFormatNumber(row.total_part);
          }
        },
        {
          data: null,
          render: function(data, type, row, meta) {
            if (row['batal'] == 1) {
              ...
              <ul class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                <li><a class="dropdown-item disabled" onclick="close_part_bp(${row.id})" href="#" readonly><i class='fa fa-arrow-right'"></i> Close Part</a></li>
                <li><a class="dropdown-item <?= $unproses == 1 ?  '' : 'disabled' ?>"" onclick="unclose_part_bp(${row.id})" href="#" readonly><i class='fa fa-arrow-left'"></i> Unclose Part</a></li>
              </ul>
            </div>`;
              } else {
                return `<div class="btn-group" role="group">
              ...
              <ul class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                <li><a class="dropdown-item <?= $edit == 1 ?  '' : 'disabled' ?>" onclick="input_part_bp(${row.id})" href="#" readonly><i class='fa fa-book'"></i> Pembebanan Part</a></li>
                <li><a class="dropdown-item <?= $proses == 1 ?  '' : 'disabled' ?>" onclick="close_part_bp(${row.id})" href="#" readonly><i class='fa fa-arrow-right'"></i> Close Part</a></li>
                <li><a class="dropdown-item disabled" onclick="unclose_part_bp(${row.id})" href="#" readonly><i class='fa fa-arrow-left'"></i> Unclose Part</a></li>
              </ul>
            </div>`;
              }
            }
  ...
  function input_part_bp($id) {
    $.ajax({
      type: "post",
      url: "<?= site_url('part_bp/input_part_bp') ?>",
      dataType: "json",
      data: {
        id: $id
      },
  ...
  function close_part_bp($id) {
    swal({
      ...
      .then((willcetak) => {
        if (willcetak) {
          $.ajax({
            url: "<?= site_url('part_bp/close_part_bp') ?>",
            type: "POST",
            data: {
              id: $id
            },
  ...
  function unclose_part_bp($id) {
    swal({
      ...
      .then((willcetak) => {
        if (willcetak) {
          $.ajax({
            url: "<?= site_url('part_bp/unclose_part_bp') ?>",
            type: "POST",
            data: {
              id: $id
            },
