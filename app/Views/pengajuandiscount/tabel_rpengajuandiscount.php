...
?> <div class=" row">
  <div class="container fluid mt-2">
    ...
    <table id="tbl-rpengajuandiscount-data" class="table table-striped" style="width:100%">
      <thead>
        <tr>
          <th width="30">No.</th>
          ...
          <th width="400">Pemesan</th>
          <th>Tipe</th>
        </tr>
      </thead>
      <tbody>
        <?php $n = 1; ?>
        <?php foreach ($pengajuandiscount as $k) : ?>
          <tr>
...
<script>
  $(document).ready(function() {
    ...
    $('#tbl-rpengajuandiscount-data1').DataTable({
      destroy: true,
      processing: true,
      serverSide: true,
      // scrollY: "400px",
      ...
      columns: [{
        ...
        },
        {
          data: null,
          render: function(data, type, row, meta) {
            return `<a href="#" onclick="detail_pengajuandiscount(${row.id})">${row.nomemo}</a>`;
          }
        },
        {
          data: 'tanggal',
        ...
        },
        {
          ...
          // render: function(data, type, row, meta) {
          //   // <a href="#${row.id}" ><button class='btn btn-sm btn-success' href='javascript:void(0)' onclick="detail(${row.id})"><i class='fa fa-eye')></i></button></a>
          //   return `<a href="#${row.id}"><button class='btn btn-sm btn-info' href='javascript:void(0)' onclick="modalpengajuandiscount(${row.id})"><i class='fa fa-book')></i></button></a>
          //   <a href="#${row.id}"><button class='btn btn-sm btn-warning' href='javascript:void(0)' onclick="edit_pengajuandiscount(${row.id})" <?= $edit == 1 ?  '' : '' ?>><i class='fa fa-edit'></i></button></a>
          //   <a href="#${row.id},${row.nomemo}"><button class='btn btn-sm btn-danger' href='javascript:void(0)' onclick="hapus_pengajuandiscount(${row.id})" <?= $hapus == 1 ?  '' : 'disabled' ?>><i class='fa fa-trash'"></i></button></a>
          //   <a href="#${row.id},${row.nomemo}"><button class='btn btn-sm btn-dark' href='javascript:void(0)' onclick="cetakpengajuandiscount(${row.id})" <?= $cetak == 1 ?  '' : 'disabled' ?>><i class='fa fa-print'"></i></button></a>`;
          // }
        },
