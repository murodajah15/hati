...
class Bahan_bpController extends Controller
{
    public function index(Request $request) //: View
    {
        ...
        $data = [
            ...
            'title' => 'Pembebanan Bahan BR',
            'userdtlmenu' => $userdtl,
            'userdtl' => Userdtl::where('cmodule', 'Pembebanan Bahan BR')->where('username', $username)->first(),
        ];
        return view('bahan_bp.index')->with($data);
    }

    public function vwobahan_bpajax(Request $request) //: View
    {
        if ($request->ajax()) {
            $data = Wo_bp::select('*');
    ...
    public function bahan_bpdetail(Request $request)
    {
        if ($request->Ajax()) {
            $username = session('username');
            ...
            return response()->json([
                'body' => view('bahan_bp.modaldetailwo', [
    ...
    public function bahanwo_bpajax(Request $request) //: View
    {
        if ($request->ajax()) {
            $data = Wo_bpd::where('nowo', $request->nowo)->where('jenis', 'BAHAN')->get();
            return Datatables::of($data)
                ->addIndexColumn()
                ->addColumn('kode1', function ($row) {
    ...
    public function bahanfaktur_bpajax(Request $request) //: View
    {
        if ($request->ajax()) {
            $data = Faktur_bpd::where('nofaktur', $request->nofaktur)->where('jenis', 'BAHAN')->get();
            return Datatables::of($data)
                ->addIndexColumn()
                ->addColumn('kode1', function ($row) {
                    $btn = $row['kode']; //'<a href="#"' . 'onclick=detail(' . $id . ')>' .  $row['kode'] . '</a>';
    ...
    public function proses_bahanwo_bp(Request $request)
    {
        if ($request->Ajax()) {
            $id = $request->id;
            ...
            if ($row_wo_bp->close == 1) {
                ...
                $update = Wo_bp::where('id', $id)->update(['close_bahan' => 1]);
                if ($update) {
                    //Create History
                    $tanggal = date('Y-m-d');
    ...
    public function unproses_bahanwo_bp(Request $request)
    {
        if ($request->Ajax()) {
            $username = session('username');
            $id = $request->id;
            ...
            return response()->json([
                'body' => view('wo_bp.modalbatalproses', [
                    'wo_bp' => $wo_bp,
                    'action' => 'unproses_bahanwo_bp_simpan',
                    'vdata' => $data,
                ])->render(),
                'data' => $data,
            ]);
    ...
    public function unproses_bahanwo_bp_simpan(Request $request)
    {
        if ($request->Ajax()) {
