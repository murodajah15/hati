...
class Kasir_tagihandController extends Controller
{
  ...
  public function kasir_tagihantambahdetail(Request $request, kasir_tagihandRequest $kasir_tagihandrequest, kasir_tagihand $kasir_tagihand)
  {
    if ($request->Ajax()) {
      ...
      if ($validate) {
        ...
        $bayar = DB::table('kasir_tagihand')->where('nokwitansi', $request->nokwitansid)->sum('bayar');
        DB::table('kasir_tagihan')->where('nokwitansi', $request->nokwitansid)->update([
          'bayar' => $bayar
        ]);
        $msg = [
  ...
  public function destroy(kasir_tagihand $kasir_tagihand, Request $request)
  {
    if ($request->Ajax()) {
      ...
      $deleted = DB::table('kasir_tagihand')->where('id', $id)->delete();
      if ($deleted) {
        $bayar = DB::table('kasir_tagihand')->where('nokwitansi', $nokwitansi)->sum('bayar');
        DB::table('kasir_tagihan')->where('nokwitansi', $nokwitansi)->update(['total' => $bayar]);
        return response()->json([
    ...
    if ($request->Ajax()) {
      ...
      if ($validate) {
        ...
        if (isset($reckasir_tagihand->nokwitansi)) {
          ...
          $kasir_tagihand->save($validate);
          $bayar = DB::table('kasir_tagihand')->where('nokwitansi', $request->nokwitansi)->sum('bayar');
          DB::table('kasir_tagihan')->where('nokwitansi', $request->nokwitansi)->update([
            'total' => $bayar,
          ]);
          $msg = [
            'sukses' => 'Data berhasil di tambah', //view('tbbarang.tabel_barang')
