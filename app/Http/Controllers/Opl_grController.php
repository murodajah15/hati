...
class Opl_grController extends Controller
{
    public function index(Request $request) //: View
    {
        ...
        $data = [
            ...
            'title' => 'Pembebanan OPL GR',
            'userdtlmenu' => $userdtl,
            'userdtl' => Userdtl::where('cmodule', 'Pembebanan OPL GR')->where('username', $username)->first(),
        ];
        return view('opl_gr.index')->with($data);
    }

    public function vwoopl_grajax(Request $request) //: View
    {
        if ($request->ajax()) {
            $data = Wo_gr::select('*');
    ...
    public function opl_grdetail(Request $request)
    {
        if ($request->Ajax()) {
            $username = session('username');
            ...
            return response()->json([
                'body' => view('opl_gr.modaldetailwo', [
    ...
    public function oplwo_grajax(Request $request) //: View
    {
        if ($request->ajax()) {
            $data = Wo_grd::where('nowo', $request->nowo)->where('jenis', 'OPL')->get();
            return Datatables::of($data)
                ->addIndexColumn()
                ->addColumn('kode1', function ($row) {
    ...
    public function oplfaktur_grajax(Request $request) //: View
    {
        if ($request->ajax()) {
            $data = Faktur_grd::where('nofaktur', $request->nofaktur)->where('jenis', 'OPL')->get();
            return Datatables::of($data)
                ->addIndexColumn()
                ->addColumn('kode1', function ($row) {
                    $btn = $row['kode']; //'<a href="#"' . 'onclick=detail(' . $id . ')>' .  $row['kode'] . '</a>';
    ...
    public function proses_oplwo_gr(Request $request)
    {
        if ($request->Ajax()) {
            $id = $request->id;
            ...
            if ($row_wo_gr->close == 1) {
                ...
                $update = Wo_gr::where('id', $id)->update(['close_opl' => 1]);
                if ($update) {
                    //Create History
                    $tanggal = date('Y-m-d');
    ...
    public function unproses_oplwo_gr(Request $request)
    {
        if ($request->Ajax()) {
            $username = session('username');
            $id = $request->id;
            ...
            $data = [
                ...
                'submenu' => 'wo_gr',
                'submenu1' => 'general_repair',
                'title' => 'Unproses opl WO General Repair',
            ...
            return response()->json([
                'body' => view('wo_gr.modalbatalproses', [
                    'wo_gr' => $wo_gr,
                    'action' => 'unproses_oplwo_gr_simpan',
                    'vdata' => $data,
                ])->render(),
                'data' => $data,
            ]);
    ...
    public function unproses_oplwo_gr_simpan(Request $request)
    {
        if ($request->Ajax()) {
