...
class TerimadController extends Controller
{
  public function destroy(Terimad $terimad, Request $request)
  {
    if ($request->Ajax()) {
      ...
      $deleted = DB::table('terimad')->where('id', $id)->delete();
      if ($deleted) {
        ...
        DB::table('terimah')->where('noterima', $noterima)->update(['biaya_lain' => $biaya_lain, 'subtotal' => $subtotal, 'total' => $total]);
    ...
    if ($request->Ajax()) {
      ...
      if ($validate) {
        ...
        if (isset($recterimad->noterima)) {
          ...
          $subtotal = DB::table('terimad')->where('noterima', $request->noterima)->sum('subtotal');
          $total = $biaya_lain + $subtotal;
          DB::table('terimah')->where('noterima', $request->noterima)->update([
