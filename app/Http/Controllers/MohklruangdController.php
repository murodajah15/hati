...
class MohklruangdController extends Controller
{
  ...
  public function mohklruangtambahdetail(Request $request, mohklruangdRequest $mohklruangdrequest, mohklruangd $mohklruangd)
  {
    if ($request->Ajax()) {
      ...
      if ($validate) {
        ...
        $subtotal = DB::table('mohklruangd')->where('nomohon', $request->nomohond)->sum('uang');
        DB::table('mohklruangh')->where('nomohon', $request->nomohond)->update([
          'subtotal' => $subtotal
        ]);
        $msg = [
  ...
  public function destroy(mohklruangd $mohklruangd, Request $request)
  {
    if ($request->Ajax()) {
      ...
      $deleted = DB::table('mohklruangd')->where('id', $id)->delete();
      if ($deleted) {
        $uang = DB::table('mohklruangd')->where('nomohon', $nomohon)->sum('uang');
        DB::table('mohklruangh')->where('nomohon', $nomohon)->update(['subtotal' => $uang]);
        return response()->json([
    ...
    if ($request->Ajax()) {
      ...
      if ($validate) {
        ...
        if (isset($recmohklruangd->nomohon)) {
          ...
          $mohklruangd->save($validate);
          $uang = DB::table('mohklruangd')->where('nomohon', $request->nomohon)->sum('uang');
          $mohklruangh = Mohklruangh::where('nomohon', $request->nomohon)->first();
          $total = $uang + $mohklruangh->materai;
          $kurang = $total - $mohklruangh->bayar;
          DB::table('mohklruangh')->where('nomohon', $request->nomohon)->update([
            'subtotal' => $uang,
            'total' => $total,
            'kurang' => $kurang,
          ]);
