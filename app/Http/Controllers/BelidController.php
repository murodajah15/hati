  ...
  public function destroy(Belid $belid, Request $request)
  {
    if ($request->Ajax()) {
      ...
      if ($deleted) {
        ...
        DB::table('belih')->where('nobeli', $nobeli)->update(['biaya_lain' => $biaya_lain, 'subtotal' => $subtotal, 'total_sementara' => $total_sementara, 'ppn' => $ppn, 'total' => $total]);
    ...
    if ($request->Ajax()) {
      ...
      if ($validate) {
        ...
        if (isset($recbelid->nobeli)) {
          ...
          $ppn = $belih->ppn;
          $subtotal = DB::table('belid')->where('nobeli', $request->nobeli)->sum('subtotal');
          $total_sementara = $biaya_lain + $subtotal + $materai;
          $total = $total_sementara + ($total_sementara * ($ppn / 100));
