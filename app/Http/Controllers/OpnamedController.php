...
class OpnamedController extends Controller
{
  public function destroy(Opnamed $opnamed, Request $request)
  {
    if ($request->Ajax()) {
      $id = $request->id;
      DB::table('opnamed')->where('id', $id)->delete();
    ...
    if ($request->Ajax()) {
      ...
      if ($validate) {
        ...
        if (isset($recopnamed->noopname)) {
          ...
          $subtotal = DB::table('opnamed')->where('noopname', $request->noopname)->sum('subtotal');
          $total = $biaya_lain + $subtotal;
          DB::table('opnameh')->where('noopname', $request->noopname)->update([
