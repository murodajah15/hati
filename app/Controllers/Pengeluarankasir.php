...
use App\Models\PengeluarankasirModel;
...
class Pengeluarankasir extends BaseController
{
  protected $pengeluarankasirModel, $memombrModel, $tbagamaModel, $tbsalesModel, $tbmodelModel, $tbcustomerModel, $tbbankModel, $hisuserModel;
  public function __construct()
  {
    $this->pengeluarankasirModel = new PengeluarankasirModel();
  ...
  public function index()
  {
    $data = [
      'menu' => 'transaksi',
      'submenu' => 'pengeluarankasir',
      'title' => 'Pengeluaran Kasir',
      'pengeluarankasir' => $this->pengeluarankasirModel->orderBy('nomor', 'desc')->findAll() //$wo
    ];
    echo view('pengeluarankasir/index', $data);
  }
  ...
  public function ajaxLoadData()
  {
    ...
    $data = $this->pengeluarankasirModel->tampilData($request, $katakunci, $start, $length);
    $jumlahData = $this->pengeluarankasirModel->tampilData($request, $katakunci);
    $output = array(
  ...
  public function table_pengeluarankasir()
  {
    $model = new pengeluarankasirModel();
    $data['title'] = 'Pengeluaran Kasir';
    echo view('pengeluarankasir/tabel_pengeluarankasir', $data);
  }
  ...
  public function formdetail()
  {
    if ($this->request->isAjax()) {
      ...
      $row = $this->pengeluarankasirModel->find($id);
      $data = [
        'title' => 'Detail Data Pengeluaran Kasir',
        'pengeluarankasir' => $this->pengeluarankasirModel->find($id),
      ];
      $msg = [
        'sukses' => view('pengeluarankasir/modaldetail', $data)
      ];
  ...
  public function formtambah()
  {
    if ($this->request->isAjax()) {
      $data = [
        'title' => 'Tambah Data Pengeluaran Kasir',
        'tbagama' => $this->tbagamaModel->orderBy('kode', 'asc')->findAll(),
      ];
      $msg = [
        'data' => view('pengeluarankasir/modaltambah', $data),
      ];
      echo json_encode($msg);
    } else {
      exit('Maaf tidak dapat diproses');
  ...
  public function simpandata()
  {
    if ($this->request->isAjax()) {
      ...
      if (!$valid) {
        ...
        $nomor = 'KK' . date('Y') . date('m') . sprintf("%05s", intval(substr('00000', -5)) + 1);
        $pengeluarankasir = $this->pengeluarankasirModel->buatnomor();
        if (isset($pengeluarankasir)) {
          foreach ($pengeluarankasir as $row) {
            if ($row->nomor != NULL) {
              $nomor = 'KK' . date('Y') . date('m') . sprintf("%05s", intval(substr($row->nomor, -5)) + 1);
            }
          }
        }
        ...
        $simpandata = [
          ...
          'pengeluaran' => preg_replace("/[^a-zA-Z0-9]/", "", $this->request->getVar('pengeluaran')),
          'bank_charge_pr' => $this->request->getVar('bank_charge_pr'),
          'bank_charge' => preg_replace("/[^a-zA-Z0-9]/", "", $this->request->getVar('bank_charge')),
          'total_pengeluaran' => preg_replace("/[^a-zA-Z0-9]/", "", $this->request->getVar('total_pengeluaran')),
        ...
        // var_dump($simpandata);
        $this->pengeluarankasirModel->insert($simpandata);
        $simpanhisuser = [
          'tanggal' => date('Y-m-d'),
          'dokumen' => $nomor,
          'form' => 'Pengeluaran Kasir',
          'status' => 'Tambah',
          'catatan' => '',
  ...
  public function formedit()
  {
    if ($this->request->isAjax()) {
      ...
      // $nomemo = $row['nomemo'];
      $data = [
        'title' => 'Edit Data Pengeluaran Kasir',
        'pengeluarankasir' => $this->pengeluarankasirModel->find($id),
        // 'tbagama' => $this->tbagamaModel->orderBy('kode', 'asc')->findAll(),
      ];
      $msg = [
        'sukses' => view('pengeluarankasir/modaledit', $data)
      ];
      echo json_encode($msg);
    } else {
      exit('Maaf tidak dapat diproses');
  ...
  public function updatedata()
  {
    if ($this->request->isAjax()) {
      ...
      if (!$valid) {
        ...
        // var_dump($simpandata);
        $id = $this->request->getVar('id');
        $this->pengeluarankasirModel->update($id, $simpandata);
        $simpanhisuser = [
          'tanggal' => date('Y-m-d'),
  ...
  public function formapprov_spv()
  {
    if ($this->request->isAjax()) {
      ...
      $nomemo = $row['nomemo'];
      $data = [
        'title' => 'Approval Pengeluaran Kasir',
        'pengeluarankasir' => $this->pengeluarankasirModel->find($id),
        // 'memombr' => $this->memombrModel->getnomemo($nomemo),
      ];
      $msg = [
        'sukses' => view('pengeluarankasir/approv_spv', $data)
      ];
      echo json_encode($msg);
    } else {
      exit('Maaf tidak dapat diproses');
  ...
  public function formapprov_sm()
  {
    if ($this->request->isAjax()) {
      ...
      $msg = [
        'sukses' => view('pengeluarankasir/approv_sm', $data)
      ];
      echo json_encode($msg);
    } else {
      exit('Maaf tidak dapat diproses');
  ...
  public function formapprov_dir()
  {
    if ($this->request->isAjax()) {
      ...
      $msg = [
        'sukses' => view('pengeluarankasir/approv_dir', $data)
      ];
      echo json_encode($msg);
    } else {
      exit('Maaf tidak dapat diproses');
  ...
  public function hapus($id)
  {
    ...
    $this->hisuserModel->insert($simpanhisuser);
    $this->pengeluarankasirModel->delete_by_id($id);
    session()->setFlashdata('pesan', 'Data berhasil dihapus');
    echo json_encode(array("status" => TRUE));
  }

  public function cetakpengeluarankasir($id)
  {
    $session = session();
    $row = $this->pengeluarankasirModel->find($id);
    $simpanhisuser = [
    ...
    return view('pengeluarankasir/cetakpengeluarankasir', $data);
    ...
    // $html =  view('pengeluarankasir/cetakpengeluarankasir', $data);
    // $dompdf->loadHtml($html);
    // $dompdf->setPaper('Legal', 'Potrait');
    // $dompdf->render();
    // // $dompdf->stream(); //langsung download
    // $dompdf->stream('pengeluarankasir ' . $nomor . '.pdf', array("Attachment" => false));
    // // var_dump($msg);
    // // echo json_encode($msg);
    // // } else {
    // //   exit('Maaf tidak dapat diproses');
  ...
  public function cari_data_memo()
  {
    if ($this->request->isAjax()) {
      ...
      $msg = [
        'data' => view('pengeluarankasir/modalcarimemo', $data),
      ];
      echo json_encode($msg);
    } else {
      exit('Maaf tidak dapat diproses');
  ...
  public function cari_data_tbbank()
  {
    if ($this->request->isAjax()) {
      ...
      $msg = [
        'data' => view('pengeluarankasir/modalcaritbbank', $data),
      ];
      echo json_encode($msg);
    } else {
      exit('Maaf tidak dapat diproses');
  ...
  public function rpengeluarankasir()
  {
    $data = [
      'menu' => 'report',
      'submenu' => 'rpengeluarankasir',
      'title' => 'Laporam Memo Mobil Baru',
    ];
    echo view('pengeluarankasir/rpengeluarankasir', $data);
  }

  public function table_rpengeluarankasir()
  {
    if ($_POST['periode'] == 'off') {
      $data = [
        'periode' => $_POST['periode'],
        'title' => 'Laporam Memo Mobil Baru Semua Periode',
        'pengeluarankasir' => $this->pengeluarankasirModel->findAll(),
      ];
    } else {
      ...
      $data = [
        ...
        'tanggal2' => $_POST['tanggal2'],
        'periode' => $_POST['periode'],
        'pengeluarankasir' => $this->pengeluarankasirModel->getperiode($tanggal1, $tanggal2),
      ];
    }
    // var_dump($data);
    echo view('pengeluarankasir/tabel_rpengeluarankasir', $data);
  }
}
