...
class Wo_bpModel extends Model
{
  ...
  public function tampilData($request, $katakunci = null, $start = 0, $length = 0)
  {
    ...
    if ($katakunci) {
      ...
      $builder = $builder->orderBy('nowo', 'asc');
    }
    ...
    if ($order == 0) {
      $builder = $builder->orderBy('nowo', 'desc');
    } else if ($order == 1) {
      $builder->orderBy('nowo', $order_dir);
    } else if ($order == 2) {
  ...
  public function getnowo($nowo = false)
  {
    ...
    return $this->where('nowo', $nowo)->findAll();
  }
