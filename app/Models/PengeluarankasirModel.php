...
class PengeluarankasirModel extends Model
{
  protected $table      = 'pengeluarankasir';
  // protected $primaryKey = 'id';
  protected $useTimestamps = true;
  protected $allowedFields = [
    'tanggal', 'nomor', 'nomemo', 'tglmemo', 'nospk', 'tglspk', 'kdcustomer', 'nmcustomer', 'piutang', 'pengeluaran', 'kdacc', 'nmacc', 'bank_charge_pr',
    'bank_charge', 'total_pengeluaran', 'keterangan', 'cara_bayar', 'kdbank', 'nmbank', 'pemegang_kartu', 'kdjnkartu', 'nmjnkartu', 'norek', 'nocek',
    'tglcek', 'tgljttempocek', 'valid', 'batal', 'user',
  ];

  public function tampilData($request, $katakunci = null, $start = 0, $length = 0)
  {
    $builder = $this->table('pengeluarankasir');
    if ($katakunci) {
  ...
  {
    $builder = $this->db->table("pengeluarankasir");
